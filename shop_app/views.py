...
def products(request):
    ...
    if request.method=="POST":
        ...
        products = Product.objects.filter(  Q(category__icontains=search) | 
...
def add_to_cart(request,id):
    ...
    if request.method=="POST":
        ...
        if CartItem.objects.filter(productv=productv, user=user):
            messages.error(request, "Item is already in Cart. You can adjust its quantity.")
        else:
            CartItem.objects.create(user=user, productv=productv, quantity=quantity)
...
def remove_cart_item(request, item_id):
    cart_obj = CartItem.objects.get(id=item_id)
    cart_obj.delete()
...
def update_cart(request):
    ...
    cart_obj = CartItem.objects.get(id=product_id)
...
def checkout(request):
    if request.method == 'POST':
        ...
        ordered_item = orderedItem.objects.create(user=user, address=add, total_price=cart_total)
        ordered_item.products.set(items)
